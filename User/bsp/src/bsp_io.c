...
//Definición del puerto de salida de señal PC0-LAMP_LAT PC1-LAMP_CLK PC2-LAMP_DAT 
...
//Definición del puerto de entrada de detección del estado de la señal
//DET_LOAD PC14 
...
//Entrada de señal del tablero de control manual
...
void bsp_InitIO(void)
{
	...
	*/
    ...
    //Definición del puerto de salida de señal PC0-LAMP_LAT PC1-LAMP_CLK PC2-LAMP_DAT PC3-WIFI_RST PC13-PPS
    ...
    //Detección de señal DET_IN (PE6), entrada de panel de control manual KEY_IN (PE5)
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	  /* IO口最大速度 */
	GPIO_InitStructure.GPIO_Pin = DET_IN_PIN | KEY_IN_PIN | 0X001F;
	GPIO_Init(GPIOE, &GPIO_InitStructure);
    
   /* Configuración de inicialización de pines (PD8-15 in0 - in8) */
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
	GPIO_InitStructure.GPIO_Speed=GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Pin  = 0xff00;
	GPIO_Init(GPIOD, &GPIO_InitStructure);
    
    /* Configuración inicial del pin BEEP (PD1) */
	...
	if(temp_var1 != 0)
	{
		...
		for(i = 0; i < 8; i++)
		{
			if((temp_var1 & temp_var2)==temp_var2)
			{
				...
				{
					if(++Input->counter[i] >= 10)//La señal dura 100ms
					{
...
void lamp_state_detect(void)
{
    ...
    DET_LOAD_Set();for(n=0;n<2;n++);//Carga de lectura variable 72M_ ciclo de instrucción único 13,88 ns, 50M - 20ns, 165 al menos 16ns
...
uint8_t manual_scan(void)
{
    ...
    KEY_LOAD_Set();for(n=0;n<5;n++);//Carga de lectura variable 72M_ ciclo de instrucción único 13,88 ns, 50M - 20ns, 165 al menos 16ns
