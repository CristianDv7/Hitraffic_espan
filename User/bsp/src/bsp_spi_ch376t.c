/*
*********************************************************************************************************
*	Nombre del módulo: chip de gestión de archivos de interfaz SPI
...
/* Línea de selección de chip CH376T, ajuste bajo para seleccionar, ajuste alto para no seleccionar */
...
/*
*********************************************************************************************************
*	Nombre de la función: sf_ConfigGPIO
*	Descripción de la función: Configure el chip select GPIO de serial Flash. Establecer como salida push-pull
...
//    Ch376s_SetSdoInt(0x10); //Configure SDO para deshabilitar la función de interrupción
...
// Ch376s_SetSdoInt: establecer el modo de interrupción SDO
// Parámetros: 0x10 deshabilita la salida de interrupción SDO; 0x90 salida de solicitud de interrupción cuando la selección de chip no es válida
...
{
    uint8_t Status;
    if(Usb_status.fox == CheckConnect)//comprobar la conexión usb
    {
        ...
        {
            if(Usb_status.OperFlag == FALSE)//La operación de comando falló
            {
                ...
                else //Si no hay ninguna operación aquí, no continuará verificando si está conectado
                {
                    Usb_status.SendCmdFlag = FALSE;
                    Usb_status.Count = 0; 
                }
            }
            else//La operación de comando tuvo éxito
            {
    ...
    else if(Usb_status.fox == DiskMount)
    {
        ...
        else //se ha enviado el comando, luego verifique el valor de estado
        {
            if(Usb_status.OperFlag == FALSE)
            {
                ...
                else//Si no hay ninguna operación aquí, no continuará verificando si se ha montado
                {
                    Usb_status.SendCmdFlag = FALSE;
                    Usb_status.Count = 0; 
                }
...
uint8_t	CH376FileOpenDir(uint8_t* PathName, uint8_t StopName)
{
    ...
    if(s != ERR_OPEN_DIR)//0x41 Si el directorio no se abre correctamente, hay un problema
    {
        ...
        else return(s); // error de operación
    }
...
uint8_t	CH376FileOpenDir(uint8_t* PathName, uint8_t StopName)
{
    ...
    while(1)
    {
		...
		if(i && i!=StopName)
        {
			if(s != ERR_OPEN_DIR)//0x41 Si el directorio no se abre correctamente, hay un problema
            {
				if(s == USB_INT_SUCCESS) return(ERR_FOUND_NAME); // Abrió con éxito el archivo printf("USB_INT_SUCCESS");
				else if(s == ERR_MISS_FILE) return(ERR_MISS_DIR);// El archivo o directorio no existe printf("ERR_MISS_FILE");
				else return(s); // error de operación printf("otro error %x",s);
			}
			s = i;
		}
		else return(s);
...
//El nombre del directorio no puede exceder los 8 bytes y debe contener letras mayúsculas, números y caracteres chinos
...
void host(void)
{
	...
	while(1)
    {
       ...
       //lectura de datos del canal
		strcpy(buf,"\\GROUP.BIN");//Consulte el manual de CH376T, el nombre del archivo debe ser números o letras mayúsculas, ¡no letras minúsculas!
		status = CH376FileOpenPath((uint8_t*)buf);
		...
		else
        {
			mStopIfError(status);
			TotalCount = 128;
			printf( "Los primeros %d caracteres leídos del archivo son:\n",TotalCount);
			while(TotalCount)
            {
				i = TotalCount;
...
UnknownUsbDevice:
		...
		while(CH376DiskConnect() == USB_INT_SUCCESS) /*Compruebe si el disco U está conectado, espere a que se extraiga el disco U */
        {
			bsp_DelayMS(1000);
		}
		bsp_LedOff(LED_ERROR);
