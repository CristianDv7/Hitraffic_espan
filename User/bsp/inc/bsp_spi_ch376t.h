...
/* estructura, las variables miembro deben ser volátiles, de lo contrario puede haber problemas cuando el compilador de C optimiza */
typedef struct
{
    volatile uint8_t fox;      /* proceso zorro */
	volatile uint8_t SendCmdFlag;     /* comprobar estado */
	volatile uint8_t Cmd;		/* comando de operación usb */
	volatile uint8_t Status;	/* estado de retorno esperado */
    volatile uint8_t OperFlag;  /* resultado de la operación */
	volatile uint32_t Count;	/* encimera */
    volatile uint32_t MaxCount; /* recuento máximo */
}USB_STATUS;
