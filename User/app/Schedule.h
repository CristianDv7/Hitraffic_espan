...
typedef struct
{
    ...
    uint8_t Day;        //3         bit0 reservado bit 1-7 domingo-sábado
    uint8_t Date[4];    //4,5,6,7   bit0 reservado bit 1-31
}DateType;

typedef struct
{
    uint8_t     Num;        //1      número de plan de envío
    DateType    Date;       //2-8   Datos de la fecha del plan de envío
    uint8_t     PlanNum;    //9      número de horario
}ScheduleType; //Definición del plan de programación
...
typedef struct
{
    ...
    uint8_t Reserve[7];
}ScheduleTable; //Horario de programación 8 + 40*9 = 368 = 0x0170




extern ScheduleType     ScheduleNow;
extern ScheduleTable    ScheduleTab;   //Horario de programación
